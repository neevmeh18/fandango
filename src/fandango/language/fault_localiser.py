...
"+OK POP3 server ready\r\nUSER debug@localdomain.test\r\n+OK\r\nPASS NEWpass123\r\n+OK All good\r\nLIST\r\n+OK 1 message\r\nQUIT\r\n+OK See you\r\n"
    ...
    print("Number of inputs successfully passed", counter)
    ...
    def run_fandango(fan_file, population_size):
        error_flag = False
        print(f"\n[validate] Running Fandango on grammar: {fan_file}")
        error_signals = []
        ...
        try:
            ...
            error_signals.append({"type": "parse_error", "line": str(e)})
        ...
        while count < 5:
            try:
                ...
                error_flag = True
                # If generator throws: treat like server error (same as before)
                msg = str(e)

                if "-ERR Unknown command" in msg:
                    error_signals.append({"type": "client_error", "line": msg})
                elif "Could not parse received message fragments into predicted NonTerminals" in msg:
                    error_signals.append({"type": "server_error", "line": msg})
                elif "Timed out while waiting for next message fragment from Server" in msg:
                    error_signals.append({"type": "server_error", "line": msg})
                elif "Timed out while waiting for message from remote party" in msg:
                    error_signals.append({"type": "client_error", "line": msg})
                else:
                    pass
        
            print("[validate] Completed iteration", count)
            count += 1
        ...
        return error_signals, error_flag



    signals, error_flag = run_fandango(fan_file_path, 20)
    print("\n=== Collected Error Signals ===")
    for sig in signals:
        print(sig["type"], "->", sig["line"])

    if counter != len(test_inputs):
    ...
    return ranked_nonterminals, max_positions, error_flag 
